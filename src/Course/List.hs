...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight f0 (Full Nil)
  where
    f0 Empty _ = Empty
    f0 _ Empty = Empty
    f0 (Full a) (Full as) = Full (a:.as)
...
find ::
  ...
  -> List a
  -> Optional a
find p = foldRight f0 Empty
  where
    f0 a | p a = const (Full a)
         | otherwise = id
